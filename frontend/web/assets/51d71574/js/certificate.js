...
dvizh.certificate = {
    init: function () {
        ...
        $(document).on('click', '[data-role=remove-target-item]', function () {
            ...
            var data = {
                'certificateId': dvizh.certificate.getUrlVar()["id"],
                'targetModel': $(this).data('target-model'),
                'targetModelId': $(this).data('target-model-id')
            };
            dvizh.certificate.deleteTargetItem(delete_url, data, block);
            $(currentBlock).remove();
        });
