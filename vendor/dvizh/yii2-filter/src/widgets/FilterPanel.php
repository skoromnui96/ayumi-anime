...
class FilterPanel extends \yii\base\Widget
{
    ...
    public function run()
    {
        ...
        foreach($filters as $filter) {
            if(empty($this->itemId) || in_array($this->itemId, $filter->selected)) {
                ...
                if($filter->type == 'range') {
                    ...
                    $variantsListWithNull = ['' => $filter->name];
                    ...
                    $block = Html::dropDownList($fieldName, $value, $variantsListWithNull, ['class' => 'dropdown']);
                } else {
                    foreach($variants as $variant) {
                        $checked = false;
        ...
        if($return) {
            ...
            return Html::tag('form', implode('', $return), ['data-resulthtmlselector' => $this->resultHtmlSelector, 'name' => 'dvizh-filter', 'action' => $action, 'class' => 'dvizh-filter dropdown']);
        }
