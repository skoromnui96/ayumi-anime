...
<div class="order-view">
    ...
    if($model->first_name) {
        $detailOrder['attributes'][] = 'first_name';
    }

    if($model->last_name) {
        $detailOrder['attributes'][] = 'last_name';
    }

    if($model->gender) {
        $detailOrder['attributes'][] = 'gender';
    }
    ...
    if($model->address2) {
        $detailOrder['attributes'][] = 'address2';
    }

    if($model->country) {
        $detailOrder['attributes'][] = 'country';
    }

    if($model->state) {
        $detailOrder['attributes'][] = 'state';
    }

    if($model->city) {
        $detailOrder['attributes'][] = 'city';
    }

    if($model->postal_code) {
        $detailOrder['attributes'][] = 'postal_code';
    }

    if($model->same_address) {
        $detailOrder['attributes'][] = 'same_address';
    }

    if($model->news_subscription) {
        $detailOrder['attributes'][] = 'news_subscription';
    }
