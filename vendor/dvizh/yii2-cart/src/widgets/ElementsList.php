...
class ElementsList extends \yii\base\Widget
{
    ...
    public $elementViewVideo = 'elementListRowVideo';
    ...
    private function _row($item)
    {
        ...
        if($product->className() == 'dvizh\shop\models\Product') {
            $allOptions = $product->getCartOptions();
            $cartElName = $product->getCartName();
            $image = $product->getImage()->getUrl('150x150');
            $product_id = $product->id;

            if ($fields = $product->fields) {
                foreach ($fields as $field) {
                    $value = $product->getField($field->slug);
                    if (is_array($value)) {
                        $value = implode(', ', $value);
                    }
                    if (!empty($value)) {
            ...
            $otherField = Html::ul($lis, ['class' => 'properties']);
            ...
            $image = $product->getImage()->getUrl('150x150');
            return $this->render($this->elementViewVideo, [
                'model' => $item,
                'name' => $cartElName,
                'showCountArrows' => $this->showCountArrows,
                'cost' => $this->_getCostFormatted($item->getCost(false)),
